...
class App:
  ...
  def App(self) -> None:
    ...
    barra_pesquisa = CTkEntry(master= central, placeholder_text="Pesquise: Ex: Frango, Coca, Sabão", text_color=("#808080", "#A0A0A0"), corner_radius=2, fg_color = "transparent")
    barra_pesquisa.place(relx =0.5, rely=0.028,relwidth = 0.9,relheight = 0.055,anchor="center")
    
    lista = CTkButton(master= central, text="Lista",corner_radius =0, fg_color=("#ADB4B4", "#2C2C2C"),hover_color=("#C7C7C7", "#474747"))
    lista.place(relx= 0.024, rely=0.028, relwidth = 0.05,relheight = 0.055, anchor="center")
    
    buscar = CTkButton(master= central, text="Buscar",corner_radius =0, fg_color=("#ADB4B4", "#2C2C2C"),hover_color=("#C7C7C7", "#474747"))
    ...
    def scroll_windows(event):
        scroll._parent_canvas.yview_scroll(-int(event.delta / 120), "units")
    ...
    if so == "Windows":
      ...
      scroll.bind("<Leave>", lambda e: scroll.unbind_all("<Button-4>"))
      scroll.bind("<Leave>", lambda e: scroll.unbind_all("<Button-5>"))
